...
// Nearest custom and New England trip logic
...
void customTrip(const StadiumGraph& graph, Stadium *start, QStringList &visited, int &distance, QString destination, int& distanceToDestination)
{
    ...
    //Search distances container for destination city
    for (it = dist.begin(); it != dist.end(); it++)
    {
        //if the city matches our destinatino
            if(it.key() == destination)
            {
                qDebug() << it.key() << "\t" << it.value();
                //add the distance
                distanceToDestination = distanceToDestination + it.value();
            }
    }
    return;
}
